 ...
 *
 * 【予測可能な状態変更】
 * ストアでは、状態の更新は必ずリデューサー関数を通じて行われます。
 * リデューサーは純粋関数であり、同じ入力に対して常に同じ出力を返します。
 * これにより、状態の変更が予測可能になり、副作用のない安定したアプリケーションを
 * 構築できます。
 *
 * 【時間旅行デバッギング（Time Travel Debugging）】
 * 状態の変更が明示的なアクションとリデューサーを通じて行われるため、
 * すべての状態変更の履歴を記録できます。これにより、アプリケーションの
 * 状態を過去の任意の時点に戻す「時間旅行デバッギング」が可能になります。
 * これは複雑なバグを特定する際に非常に強力なツールです。
 *
 * 【スケーラビリティと保守性】
 * ストアパターンは、アプリケーションが大きくなっても状態管理の複雑さを
 * 制御できるように設計されています。状態の更新ロジックが集中管理されるため、
 * 新機能の追加や既存機能の変更が容易になります。
 *
 * また、状態とUIが明確に分離されるため、それぞれを独立してテストできます。
 * これにより、テストの容易性と信頼性が向上します。
 */
